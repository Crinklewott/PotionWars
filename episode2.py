...
def init_episode_2_1():
    ...
    ep2_guild_wake_up = conversation.Node( 322 )

    def ep2_guild_wake_up_quip_function():
        ep2_guild_wake_up.quip = universal.format_text_no_space([['''''']])
        ep2_guild_wake_up.quip = ""
        if "extrovert" in universal.state.player.keywords:
            ep2_guild_wake_up.children = ep2_guild_wake_up_extrovert.children
            conversation.say_node(ep2_guild_wake_up_extrovert)
            
        elif "introvert" in universal.state.player.keywords:
    ...
    ep2_guild_wake_up_extrovert = conversation.Node( 323 )

    def ep2_guild_wake_up_extrovert_quip_function():
        ep2_guild_wake_up_extrovert.quip = universal.format_text_no_space([[''''''],
...
['''Ildri and Alondra are standing at the counter, kneading several piles of dough. Ildri glances over her shoulder at ''', universal.state.player.name, '''. She wipes her hands on her apron, then turns and puts them on her hips. "Well, well. Look whose finally up."''']])
        ep2_guild_wake_up_extrovert.add_player_comment('''"I didn't wake you, did I?"''')
        ep2_guild_wake_up_extrovert.add_child(ep2_guild_wake)
        ep2_guild_wake_up_extrovert.add_player_comment('''"Go stick your ugly face in someone else's business, you stupid old busybody."''')
        ep2_guild_wake_up_extrovert.add_child(ep2_guild_snap)
        ep2_guild_wake_up_extrovert.add_player_comment('''"Ugh."''')
        ep2_guild_wake_up_extrovert.add_child(ep2_guild_grunt)

    
    ep2_guild_wake_up_extrovert.quip_function = ep2_guild_wake_up_extrovert_quip_function
    ep2_guild_wake = conversation.Node( 324 )

    def ep2_guild_wake_quip_function():
        ep2_guild_wake.quip = universal.format_text_no_space([['''"Oh, you'd know if you'd woken me, trust me," says Ildri. She glances over her shoulder at Alondra, who is starting to move her dough onto a baking pan. She spins around and gives the girl a sharp swat. "You call that done? You put that in the oven, and half the loaf will be burned, the other hardly cooked!"'''],
...
['''Ildri jerks her head towards a small platter on the far end of the counter. "Some biscuits over there, a few small pieces of fruit, left over from the winter stores. Also a pitcher of water, sanitized by Paloma before her meeting with Adrian."''']])
        ep2_guild_wake.add_player_comment('''Blanch''')
        ep2_guild_wake.add_child(ep2_guild_beer)
        ep2_guild_wake.add_player_comment('''"Good. Thought of beer makes me want to hurl."''')
        ep2_guild_wake.add_child(ep2_guild_water)

    
    ep2_guild_wake.quip_function = ep2_guild_wake_quip_function
    ep2_guild_beer = conversation.Node( 325 )

    def ep2_guild_beer_quip_function():
        ep2_guild_beer.quip = universal.format_text_no_space([['''Ildri shakes her finger at the Taironan. "Don't give me that face young ''', person.manlady(), '''. Drinking watered down beer is acceptable when you don't have potable water, but this is perfectly clean."'''],
['''''', universal.state.player.name, ''' grimaces, but doesn't try to argue. Instead, ''', person.heshe(), ''' makes ''', person.hisher(), ''' way towards the far end of the counter. ''', person.HeShe(), ''' snatches up half a dozen of the biscuits, and grabs a few slices of dried apples. ''', person.HeShe(), ''' opens a small container of conserves, and slathers some on the biscuits. Then, ''', person.heshe(), ''' wolfs the food down, glancing sideways at the pitcher of water. ''', person.HeShe(), ''' glances about, and ''', person.hisher(), ''' eyes light on the small box (kept chilled with a charm maintained by Airell) containing the beer. ''', person.HeShe(), ''' looks between the pitcher of water, and the container, then glances at Ildri, whose back is towards the icebox. The buxom cook is currently explaining the finer details of dough kneading to Alondra, who listens with a slightly bewildered look on her face.''']])
        ep2_guild_beer.add_player_comment('''Grab some beer! Ildri won't notice.''')
        ep2_guild_beer.add_child(ep2_guild_beer)
        ep2_guild_beer.add_player_comment('''Just drink the water. It'll probably make you feel better anyway.''')
        ep2_guild_beer.add_child(ep2_guild_water)

    
    ep2_guild_beer.quip_function = ep2_guild_beer_quip_function
    ep2_guild_take_beer = conversation.Node( 326 )

    def ep2_guild_take_beer_quip_function():
        ep2_guild_take_beer.quip = universal.format_text_no_space([['''''']])
        ...
        if universal.state.player.stealth() +  random.randint(4)  >= 8:
            ...
            conversation.say_node(ep2_guild_take_beer_failure.index)


    
    ep2_guild_take_beer.quip_function = ep2_guild_take_beer_quip_function
    ep2_guild_take_beer_success = conversation.Node( 327 )
...
['''''', universal.state.player.name, ''' finishes ''', person.hisher(), ''' breakfast, sighing as the delicious food fills ''', person.hisher(), ''' empty belly. ''', person.HisHer(), ''' headache has already faded to manageable levels. Perhaps it's time to see if Adrian has a job for ''', person.himher(), '''? It's been a month, and ''', person.heshe(), ''' has had nothing to do except help get the guild back together (which hardly counts)! There'd better be something today, or ''', universal.state.player.name, ''' just might just go crazy.''']])
    ...
    ep2_guild_take_beer_failure = conversation.Node( 328 )

    def ep2_guild_take_beer_failure_quip_function():
        ep2_guild_take_beer_failure.quip = universal.format_text_no_space([['''''', universal.state.player.name, ''' watches Ildri and Alondra for a moment. The two seem pretty absorbed in the intricacies of dough kneading. ''', person.HeShe(), ''' turns, and hastens over to the icebox, quickly yanking it open, and going for the beer pitcher.'''],
...
['''''', universal.state.player.name, ''' swallows uneasily, and glances over ''', person.hisher(), ''' shoulder. Ildri ''']])
        if ildri.towers_over(universal.state.player):
            ep2_guild_take_beer_failure.quip = universal.format_text_no_space([[ep2_guild_take_beer_failure.quip,'''towers over''']])
        else:
            ep2_guild_take_beer_failure.quip = universal.format_text_no_space([[ep2_guild_take_beer_failure.quip,'''stands behind''']])
        ep2_guild_take_beer_failure.quip = universal.format_text_no_space([[ep2_guild_take_beer_failure.quip,''' ''', person.himher(), ''' with her arms crossed and an annoyed look on her face.''']])
        ep2_guild_take_beer_failure.add_player_comment('''Maybe if you smile innocently enough, she won't notice your arm sticking out of the icebox!''')
        ep2_guild_take_beer_failure.add_child(ep2_innocent_smile)
        ep2_guild_take_beer_failure.add_player_comment('''"Umm...Ildri. Hi. Nice weather we're having, huh?"''')
        ep2_guild_take_beer_failure.add_child(ep2_greet_ildri)
        ep2_guild_take_beer_failure.add_player_comment('''"So I'm drinking beer. I'm an adult, I can drink whatever I want. Now leave me alone!"''')
        ep2_guild_take_beer_failure.add_child(ep2_beer_bratty)

    
    ep2_guild_take_beer_failure.quip_function = ep2_guild_take_beer_failure_quip_function
    ep2_innocent_smile = conversation.Node( 329 )

    def ep2_innocent_smile_quip_function():
        ep2_innocent_smile.quip = universal.format_text_no_space([['''''', universal.state.player.name, ''' puts on ''', person.hisher(), ''' best smile of pure innocence.'''],
...
['''''']])
        if ildri.taller_than(universal.state.player):
            ep2_innocent_smile.quip = universal.format_text_no_space([[ep2_innocent_smile.quip,'''As Ildri continues to glare down at ''', universal.state.player.name, ''', ''', universal.state.player.name, ''' becomes acutely aware of just how tall the guild mistress is. Very tall. Tremendously tall. The kind of tall that goes with lots of strength. The kind of strength that interferes with sitting for several days.''']])
        else:
            ep2_innocent_smile.quip = universal.format_text_no_space([[ep2_innocent_smile.quip,'''Despite their comparable heights, Ildri seems to tower over ''', universal.state.player.name, ''', as if the woman had grown six inches in the past six seconds. Kind of like Nana. How do they do that?''']])
        ep2_innocent_smile.quip = universal.format_text_no_space([[ep2_innocent_smile.quip,'''''']])
        ep2_innocent_smile.children = ep2_ildri_scolding.children
        conversation.say_node(ep2_ildri_scolding.index)


    
    ep2_innocent_smile.quip_function = ep2_innocent_smile_quip_function
    ep2_greet_ildri = conversation.Node( 330 )

    def ep2_greet_ildri_quip_function():
        ep2_greet_ildri.quip = universal.format_text_no_space([[''''''],
...
['''"Oh." ''', universal.state.player.name, ''' laughs uneasily, ''', person.hisher(), ''' bottom frantically clenching and unclenching. "That's, um, yeah. That makes sense. I mean, who doesn't love that?"''']])
        ep2_greet_ildri.children = ep2_ildri_scolding.children
        conversation.say_node(ep2_ildri_scolding.index)


    
    ep2_greet_ildri.quip_function = ep2_greet_ildri_quip_function
    ep2_ildri_scolding = conversation.Node( 331 )

    def ep2_ildri_scolding_quip_function():
        ep2_ildri_scolding.quip = universal.format_text_no_space([['''''']])

    
    ep2_ildri_scolding.quip_function = ep2_ildri_scolding_quip_function
    ep2_beer_bratty = conversation.Node( 332 )

    def ep2_beer_bratty_quip_function():
        ep2_beer_bratty.quip = universal.format_text_no_space([['''''', universal.state.player.name, ''' has barely finished the sentence, before ''', person.heshe(), ''' finds ''', person.himselfherself(), ''' tucked under Ildri's arm, ''']])
        if ildri.towers_over(universal.state.player):
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''''', person.hisher(), ''' feet dangling in mid-air as Ildri hoists ''', person.himher(), ''' completely off the ground.''']])
        else:
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''''', person.hisher(), ''' feet scrambling against the floor.''']])
        ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,''' The woman's large, hard hand starts smashing into ''', universal.state.player.name, ''''s''', ''' ''', universal.state.player.muscle_adj(), ''' bottom. ''', universal.state.player.name, ''' squeals and kicks as hot pain blooms in ''', person.hisher(), ''' ''', universal.state.player.clad_bottom(pajama=True), ''' cheeks.'''],
['''"Not when it's my beer you can't," says Ildri, her punishing hand making ''', universal.state.player.name, ''''s''', ''' ''', universal.state.player.bum_adj(), ''' bottom ''']])
        if universal.state.player.is_soft():
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''ripple''']])
        elif universal.state.player.is_fit():
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''jiggle''']])
        else:
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''jump''']])
        ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''. "You've spent almost every night of the past month drinking like a fish, and I'll be cursed if I let you drink during the day too!"'''],
...
['''"That's not-oww!" ''']])
        if ildri.taller_than(universal.state.player) or ildri.height == universal.state.player.height:
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''''', universal.state.player.name, ''' kicks ''', person.hisher(), ''' dangling feet.''']])
        else:
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''''', universal.state.player.name, ''' stomps ''', person.hisher(), ''' foot against the ground.''']])
        ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,''' "I don't want to fix some ugly, smelly kitchen! I want to adventure!"'''],
['''Alondra gasps.'''],
...
['''Ildri puts her foot on the stool, swings ''', universal.state.player.name, ''' around, lifts the Taironan a few more inches, and drapes ''', person.himher(), ''' across her lap. ''', universal.state.player.name, ''''s''', ''' arms and legs dangle uselessly, ''', person.hisher(), ''' hands and feet ''']])
        if universal.state.player.is_average_or_shorter():
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''hanging well above the floor.''']])
        else:
            ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,'''a scant few inches above the floor.''']])
        ep2_beer_bratty.quip = universal.format_text_no_space([[ep2_beer_bratty.quip,''''''],
...
['''Ah, well. Perhaps it's time to see if Adrian has a job for ''', person.himher(), '''? It's been a month, and ''', person.heshe(), ''' has had nothing to do except help get the guild back together (which hardly counts)! There'd better be something today, or ''', universal.state.player.name, ''' just might just go crazy.''']])
        ...
        universal.state.player.marks.append(''.join(['''''', universal.state.player.name, ''''s''', ''' ''', universal.state.player.bum_adj(), ''' ''', universal.state.player.muscle_adj(), ''' bottom is marred by several long, broad marks in the shape of a razor strop. ''', person.HeShe(), ''' gives one of the marks a tender rub, and makes ''', person.himselfherself(), ''' a silent promise never to raid Ildri's beer without her permission again.''']))

    
    ep2_beer_bratty.quip_function = ep2_beer_bratty_quip_function
    ep2_guild_water = conversation.Node( 333 )

    def ep2_guild_water_quip_function():
        ep2_guild_water.quip = universal.format_text_no_space([['''''']])

    
    ep2_guild_water.quip_function = ep2_guild_water_quip_function
    ep2_guild_snap = conversation.Node( 334 )

    def ep2_guild_snap_quip_function():
        ep2_guild_snap.quip = universal.format_text_no_space([['''''']])

    
    ep2_guild_snap.quip_function = ep2_guild_snap_quip_function
    ep2_guild_grunt = conversation.Node( 335 )

    def ep2_guild_grunt_quip_function():
        ep2_guild_grunt.quip = universal.format_text_no_space([['''''']])

    
    ep2_guild_grunt.quip_function = ep2_guild_grunt_quip_function
    ep2_guild_wake_up_introvert = conversation.Node( 336 )

    def ep2_guild_wake_up_introvert_quip_function():
        ep2_guild_wake_up_introvert.quip = universal.format_text_no_space([['''''']])

    
    ep2_guild_wake_up_introvert.quip_function = ep2_guild_wake_up_introvert_quip_function
    ep2_marias_wake_up = conversation.Node( 337 )

    def ep2_marias_wake_up_quip_function():
        ep2_marias_wake_up.quip = universal.format_text_no_space([['''''']])
